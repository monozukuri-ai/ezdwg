...
def test_ac1027_read_and_inspect_basics() -> None:
    path = SAMPLES / "line_2013.dwg"
    doc = ezdwg.read(str(path))

    assert doc.version == "AC1027"
    assert doc.decode_version == "AC1027"
    assert doc.decode_path == str(path)
    assert len(list(doc.modelspace().query("LINE"))) == 1


def test_ac1027_entity_counts_match_paired_dxf() -> None:
    cases = [
        ("line_2013.dwg", "LINE"),
        ("arc_2013.dwg", "ARC"),
        ("polyline2d_line_2013.dwg", "LWPOLYLINE"),
    ]
...
def test_ac1027_line_matches_paired_dxf_geometry() -> None:
    dwg_path = SAMPLES / "line_2013.dwg"
    dxf_path = SAMPLES / "line_2013.dxf"
...
def test_ac1027_arc_matches_paired_dxf_geometry() -> None:
    dwg_path = SAMPLES / "arc_2013.dwg"
    dxf_path = SAMPLES / "arc_2013.dxf"
...
def test_ac1027_lwpolyline_matches_paired_dxf_geometry() -> None:
    dwg_path = SAMPLES / "polyline2d_line_2013.dwg"
    dxf_path = SAMPLES / "polyline2d_line_2013.dxf"
...
def test_ac1027_decode_payload_shape() -> None:
    line_rows = raw.decode_line_entities(str(SAMPLES / "line_2013.dwg"))
    ...
    arc_rows = raw.decode_arc_entities(str(SAMPLES / "arc_2013.dwg"))
    ...
    lw_rows = raw.decode_lwpolyline_entities(str(SAMPLES / "polyline2d_line_2013.dwg"))
