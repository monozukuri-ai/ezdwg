...
pub fn decode_region(reader: &mut BitReader<'_>) -> Result<RegionEntity> {
    let header = parse_common_entity_header(reader)?;
    decode_region_with_header(reader, header, false, false)
}

pub fn decode_region_r14(reader: &mut BitReader<'_>, object_handle: u64) -> Result<RegionEntity> {
    let mut header = parse_common_entity_header_r14(reader)?;
    if header.handle == 0 {
        header.handle = object_handle;
    }
    decode_region_with_header(reader, header, true, false)
}

pub fn decode_region_r2007(reader: &mut BitReader<'_>) -> Result<RegionEntity> {
    let header = parse_common_entity_header_r2007(reader)?;
    decode_region_with_header(reader, header, true, true)
}

pub fn decode_region_r2010(
...
fn decode_region_with_header(
    reader: &mut BitReader<'_>,
    header: CommonEntityHeader,
    allow_handle_decode_failure: bool,
    r2007_layer_only: bool,
) -> Result<RegionEntity> {
    // REGION geometry/ACIS body decode is TODO. We can still expose metadata by
