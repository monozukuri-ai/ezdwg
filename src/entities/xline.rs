...
pub fn decode_xline(reader: &mut BitReader<'_>) -> Result<XLineEntity> {
    let header = parse_common_entity_header(reader)?;
    decode_xline_with_header(reader, header, false, false)
}

pub fn decode_xline_r14(reader: &mut BitReader<'_>, object_handle: u64) -> Result<XLineEntity> {
    let mut header = parse_common_entity_header_r14(reader)?;
    if header.handle == 0 {
        header.handle = object_handle;
    }
    decode_xline_with_header(reader, header, true, false)
}

pub fn decode_xline_r2007(reader: &mut BitReader<'_>) -> Result<XLineEntity> {
    let header = parse_common_entity_header_r2007(reader)?;
    decode_xline_with_header(reader, header, true, true)
}

pub fn decode_xline_r2010(
