...
pub fn decode_dim_diameter(reader: &mut BitReader<'_>) -> Result<DimDiameterEntity> {
    ...
    if let Ok(entity) =
        decode_dim_diameter_variants_with_header(reader, header.clone(), false, &R2000_VARIANTS)
    {
        return Ok(entity);
    }
...
pub fn decode_dim_diameter_r2007(reader: &mut BitReader<'_>) -> Result<DimDiameterEntity> {
    ...
    let data_pos = reader.get_pos();
    if let Ok(entity) =
        decode_dim_diameter_variants_with_header(reader, header.clone(), true, &R2010_PLUS_VARIANTS)
    {
        return Ok(entity);
    }
    reader.set_pos(data_pos.0, data_pos.1);
...
fn decode_dim_diameter_r2010_plus_with_header(
    ...
    decode_dim_diameter_variants_with_header(
        reader,
        header,
...
fn decode_dim_diameter_variants_with_header(
    reader: &mut BitReader<'_>,
    header: CommonEntityHeader,
    allow_handle_decode_failure: bool,
